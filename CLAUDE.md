...
This is a Vue 3 SPA built with Vite, TypeScript, Tailwind CSS, and Supabase. It serves as a template for building web applications -- user-facing web apps and internal admin panels alike -- with authentication, database access, and an API client out of the box.

**Stack:** Vue 3 (Composition API) | Vite | TypeScript | Tailwind CSS v4 | Pinia | Vue Router | Supabase

## How this app fits the wider stack

This template is designed to sit in a workspace beside a mobile app and a Python API that all share **one Supabase project**:

- **Same Supabase project = same users.** Someone who signed up in the mobile app signs in here with the same account, and vice versa. Do not create a separate Supabase project for this app.
- Browser-side reads and writes go **directly to Supabase** (PostgREST + RLS) via `db()`. Server-side logic goes **through the Python API** via `api()`. The API validates the same Supabase JWTs this app sends.
- **Database migrations do NOT live in this repo.** The schema is owned by the API repo (its `supabase/` folder). If a feature here needs a new table, column, or RLS policy, write the migration there -- never click it together in a dashboard.
...
- The store provides: `user`, `session`, `isAuthenticated`, `isAdmin`, `loading`, `initError`, `signIn()`, `signUp()`, `signInWithGoogle()`, `signOut()`.
...
- Route guards are in `router/index.ts`: `meta.requiresAuth`, `meta.requiresAdmin`, and `meta.guestOnly`.
...
- `useAuthStore().isAdmin` is `true` when the signed-in user's JWT carries `app_metadata.admin = true`.
- The flag is granted manually, per admin, in the Supabase SQL editor. It cannot be set from the client -- `app_metadata` (unlike `user_metadata`) is server-controlled, which is exactly why it's safe to trust:
...
- Use `meta: { requiresAdmin: true }` for admin-only pages.
...
- **Supabase RLS**: Remember that Supabase uses Row Level Security. If queries return empty results unexpectedly, check your RLS policies -- and remember policies are written as migrations in the API repo, not clicked together in a dashboard.
- **Admin flag timing**: `app_metadata.admin` is read from the JWT, so a freshly granted admin won't see admin UI until their token refreshes. Sign out and back in.
